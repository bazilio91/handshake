...
def db_entry(in_user):
	ls=[None, None]
	logging.debug("SELECT %s,%s FROM %s WHERE %s ='%s'"%(db_username_field,db_password_field , db_table, db_username_field, in_user))
...
def auth(in_user, in_host, password):
	...
	if data==None:
		...
		logging.debug("Wrong username: %s"%(in_user))
	if in_user+"@"+in_host==data[0]+domain_suffix:
		if hashlib.md5(hashlib.md5(password).hexdigest()).hexdigest()==data[1]:
			out=True
		else:
